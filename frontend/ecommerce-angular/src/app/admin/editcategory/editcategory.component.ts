...
export class EditcategoryComponent implements OnInit{
  ...
  fetchCategoryById() {
    if(this.categoryId){
      this.apiService.getByCategoryId(this.categoryId).subscribe({
        next:(respone)=>{
          this.categoryForm.patchValue({name: respone.category.name})
        },
        error:(error)=>{
          console.log(error)
          this.message=error?.error?.message || "unable to get category by id";
          setTimeout(()=>{
              this.message=null;
            },3000)
        }
